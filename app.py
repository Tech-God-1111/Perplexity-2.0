...
# Configure the page
st.set_page_config(
    page_title="Smart AI Assistant with Perplexity Pro",
    page_icon="🤖",
    layout="wide",
    initial_sidebar_state="collapsed"
)

# API Keys - Will come from Streamlit Secrets
try:
...
- Explain complex concepts with citations
...
def main():
    ...
    with chat_container:
        for message in st.session_state.messages:
            if message["role"] == "user":
                ...
                # Add badges
                if "Perplexity AI" in content:
                    content += ' <span class="perplexity-badge">🔍 Perplexity Pro</span>'
    ...
    with col1:
        if st.button("Who is Elon Musk?"):
            user_input = "Who is Elon Musk and his latest projects?"
    with col2:
        if st.button("What is AI?"):
            user_input = "What is artificial intelligence and current trends?"
    with col3:
        if st.button("Latest Tech"):
            user_input = "Latest technology news and developments in 2025"
    with col4:
